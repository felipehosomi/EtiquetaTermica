...
namespace CVA.AddOn.Common.Util
{
    public class FormattedSearchUtil
    {
        ...
        public bool AssignFormattedSearch(string QueryName, string TheQuery, string FormID, string ItemID, string ColID = "-1", string categoryName = "Geral")
        {
            ...
            try
            {
                ...
                if (oRS.RecordCount == 0)
                {
                    ...
                    {
                        sql = SBOApp.TranslateToHana(sql);
                        oRS.DoQuery($@"SELECT TOP 1 T0.IndexID 
                                         FROM CSHS T0 
                                        WHERE T0.FormID = '{FormID}' 
                                          AND T0.ItemID = '{ItemID}' 
                                          AND T0.ColID = '{ColID}'");
        ...
        public void RemoveFormattedSearch(string queryName, string itemId, string formId, string categoryName = "Geral")
        {
            ...
            string sql = $@"SELECT IntrnalKey, QCategory 
                              FROM OUQR 
                             WHERE QName = '{queryName}' 
                               AND QCategory = {this.GetSysCatID(categoryName)}";
            sSql = SBOApp.TranslateToHana(sSql);
            oRS.DoQuery(sql);
            if (oRS.RecordCount > 0)
            {
        ...
        public bool ExistsQuery(string query)
        {
            ...
            string sql = $@"SELECT TOP 1 1 
                              FROM OUQR
                             WHERE CAST(QString AS NVARCHAR(MAX)) = '{query}'";
        ...
        public int CreateQuery(string QueryName, string TheQuery, string categoryName = "Geral")
        {
            ...
            try
            {
                ...
                string sql = $@"SELECT TOP 1 IntrnalKey
                                  FROM OUQR 
                                 WHERE QCategory = {category} 
                                   AND QName = '{QueryName}'";
                sql = SBOApp.TranslateToHana(sql);
                oRS.DoQuery(sql);
        ...
        public void AssignFormattedSearch(string v1, object documentoRateio_Get, string v2, string v3)
        {
