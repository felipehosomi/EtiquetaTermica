...
namespace CVA.AddOn.Common.DAO
{
    public class SqlDAO : IDAO
    {
        ...
        public SqlDAO()
        {
            if (String.IsNullOrEmpty(ConnectionString))
            {
                try
                {
                    ...
                    string dbUser = System.Configuration.ConfigurationManager.AppSettings["DBUserName"];
        ...
        public T FillModelFromCommand<T>(string sql)
        {
            List<T> modelList = this.FillListFromCommand<T>(sql);
            if (modelList.Count > 0)
            {
                return modelList[0];
            }
        ...
        public List<T> FillListFromCommand<T>(string sql)
        {
            List<T> modelList = new List<T>();
            T model;
            using (SqlDataReader dr = this.ExecuteReader(sql))
            ...
            foreach (PropertyInfo property in modelType.GetProperties())
            {
                ...
                foreach (Attribute attribute in property.GetCustomAttributes(true))
                {
                    ...
                    if (modelController != null)
                    {
                        ...
                        fields += String.Format(", [{0}]", modelController.ColumnName);
                    }
                    break;
                }
            }
